...
# Overlay efficiency-vs-deltaR-by-stage curves from TWO LSTNumDen files on one axis.
# Reuses the per-stage color/marker convention from lst_plot_eff_vs_deltaR_stages.py;
# the two datasets are distinguished by marker fill + line style:
#   dataset A -> filled markers, solid line
#   dataset B -> open markers,   dashed line
# Same stage keeps the same color across both datasets so CPU-vs-CUDA (or any A-vs-B)
# consistency can be read off directly.
...
parser = argparse.ArgumentParser(
    description="Overlay efficiency-vs-deltaR-by-stage from two LSTNumDen files. "
    "Dataset A = filled markers/solid line; dataset B = open markers/dashed line."
)
parser.add_argument("input_a", help="first LSTNumDen*.root file (filled markers)")
parser.add_argument("input_b", help="second LSTNumDen*.root file (open markers)")
parser.add_argument("--labels", default="A,B", help="comma-separated dataset labels for A,B [DEFAULT=A,B]")
...
parser.add_argument("-o", "--output", default="eff_vs_deltaR_stages_overlay.png", help="output image path")
...
def main():
    ...
    if len(labels) != 2:
        parser.error("--labels must have exactly two entries (for input_a,input_b)")
    ...
    # dataset A: filled marker, solid; dataset B: open marker, dashed
    dataset_kw = [
    ...
    for stage in stages:
        ...
        for infile, dkw in zip(files, dataset_kw):
            ...
            ax.errorbar(
                ...
                lw=1.0, fillstyle=dkw["fillstyle"], markerfacecolor=(
                    style["color"] if dkw["fillstyle"] == "full" else "none"),
            )
    ...
    ax.add_artist(leg1)
    # Legend 2: dataset fill/line
    ds_handles = [
        Line2D([0], [0], color="gray", marker="o", linestyle="-", markerfacecolor="gray",
               markersize=5, label=labels[0]),
        Line2D([0], [0], color="gray", marker="o", linestyle="--", markerfacecolor="none",
               markersize=5, label=labels[1]),
    ]
    ax.legend(handles=ds_handles, fontsize=8, loc="upper right", title="Dataset")

    ax.set_title("LST Efficiency vs ΔR by Stage — {} vs {} ({})".format(labels[0], labels[1], tag))
    fig.tight_layout()
